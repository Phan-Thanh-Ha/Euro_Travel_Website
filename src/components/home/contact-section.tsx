...
export default async function ContactSection() {
    ...
    return (<section className="bg-main w-full py-6 text-center px-2">
        <div className="container mx-auto grid md:grid-cols-2 grid-cols-1 text-white gap-8 text-center align-middle">
            <div className="bg-blue-default-50 flex flex-col md:flex-row gap-4 shadow-2xl">
                ...
                <div className="shadow-2xl lg:ml-4 lg:mb-4">
                    <h4 className="font-bold text-xl my-6">WEBSITE CHÍNH THỨC EUROTRAVEL</h4>
        ...
        <div className="text-center py-4 mt-4 flex md:flex-row flex-col gap-4 justify-center">
            <Link title={"Gọi tới hot line: " + setting?.Hotline} className="text-white bg-blue-default px-6 py-2 border-2 border-blue-default rounded-full font-bold hover:text-blue-default hover:bg-white hover:border-white" href={`tel:${setting?.Hotline}`}>Tổng đài: {setting?.Hotline}</Link>
            <Link title={"Chat Messenger với " + setting?.CompanyName} className="text-white bg-main px-6 py-2 border-2 border-white rounded-full font-bold hover:text-blue-default hover:bg-white" href={setting?.FacebookChat || "/"}>Chat Messenger</Link>
        </div>
    </section>);
}
