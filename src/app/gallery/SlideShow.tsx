...
const SliderShow = ({
  ...
  data: {
    ...
    TagName: string;
    TagUrl: string;
    Images: { Image: string; GaleryId: number }[];
  }[];
}) => {
          ...
          <div className="w-full h-[280px] md:h-[700px] relative shadow-2xl">
              ...
              <div className="sm:block md:flex justify-between items-center">
                ...
                <div>
                  <button className="py-1 md:py-2 px-6 text-sm md:text-xl bg-main rounded-lg hover:bg-red-500 shadow-md">
                    <Link href={tour.TagUrl}>Xem thêm</Link>
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-center gap-4 mt-4 absolute bottom-4 left-0 w-full">
              {data.map((item, i) => (
                <div
                  key={i}
                  className={`w-[40px] h-[40px] md:w-[80px] md:h-[80px] rounded-md relative ${
                ...
                >
                  {item?.Images && (
                    <Image
                      src={`${
                        envConfig.NEXT_PUBLIC_CDN + item?.Images[0].Image
                      }`}
                      alt="image"
                      fill={true}
